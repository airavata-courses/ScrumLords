...
# The root directory of services.
# Use `.` if your services are located in root.
ROOT="./services"
...
## 2. Changed services
############################################
SERVICES=$(ls ${ROOT} -l | grep ^d | awk '{print $9}')
...
if [[ $COUNT -eq 0 ]]; then
  echo -e "\e[93mNo changes detected in services. Skip triggering workflows.\e[0m"
  exit 0
fi
